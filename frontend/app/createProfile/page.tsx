  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-xl shadow-2xl max-w-sm w-full mx-4 overflow-hidden animate-in fade-in zoom-in duration-200">
        {/* Decorative header gradient - yellow for cancel */}
        <div className="h-1.5 bg-gradient-to-r from-[#EFD780] to-[#e6c15a]" />
        ...
        <div className="p-5">
          ...
          {/* Title - smaller text */}
          <h2 className="text-lg font-bold text-center text-[#996600] mb-1">
            Cancel Creation
          </h2>
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              className="flex-1 bg-gradient-to-r from-[#EFD780] to-[#e6c15a] hover:from-[#e6c15a] hover:to-[#d9b348] text-white border-0 h-9 text-sm"
            >
              Yes, Cancel
            </Button>
          </div>
...
function CreateProfileContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (Object.keys(validationErrors).length > 0) {
      ...
      // REMOVED: toast for validation errors
      return;
    }
    ...
    try {
      ...
      if (!token) {
        toast({
          title: "Error",
          description: "You need to be logged in to create a child profile",
          variant: "destructive",
        });
      ...
      if (response.status === 201) {
        ...
        // Show toast with countdown AND OK button that redirects immediately
        toast({
          title: "Success",
          description: data.message || "Child profile created successfully!",
          variant: "success",
          duration: 3000,
          onOK: () => {
            router.push("/dashboard"); // Immediate redirect on OK click
          },
        });

        // Auto-redirect after 3 seconds (handled by toast's auto-dismiss + this timeout)
        setTimeout(() => {
  ...
  const handleCancelConfirm = () => {
    toast({
      title: "Cancelled",
      description: "Profile creation cancelled",
      variant: "default",
    });
    router.push("/dashboard");
  };
                ...
                <label
                  ...
                  className={`flex items-center justify-center p-3 rounded-lg border cursor-pointer transition-all h-11 ${
                    formData.gender === option
                      ...
                      : `border-gray-300 hover:border-[#77c1e6] bg-white ${errors.gender ? "border-red-500" : ""}`
                  }`}
                >
                  <input
                    type="radio"
                ...
                <label
                  ...
                  className={`flex items-center justify-center p-3 rounded-lg border cursor-pointer transition-all h-11 ${
                    formData.jaundice === option
                      ? "bg-[#77c1e6] border-[#77c1e6] text-white"
                      : `border-gray-300 hover:border-[#77c1e6] bg-white ${errors.jaundice ? "border-red-500" : ""}`
                  }`}
                >
                  <input
                    type="radio"
                ...
                <label
                  ...
                  className={`flex items-center justify-center p-3 rounded-lg border cursor-pointer transition-all h-11 ${
                    formData.familyASD === option
                      ? "bg-[#77c1e6] border-[#77c1e6] text-white"
                      : `border-gray-300 hover:border-[#77c1e6] bg-white ${errors.familyASD ? "border-red-500" : ""}`
                  }`}
                >
                  <input
                    type="radio"
